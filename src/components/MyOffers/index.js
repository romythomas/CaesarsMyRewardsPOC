...
class MyOffers extends Component {
  ...
  updateOfferList() {
    ...
    selectedOfferSort = selectedOfferSort
      ? selectedOfferSort
      : this.getDefaultSortValue();
    this.selectedOfferSort = selectedOfferSort;
    let filteredSortedOffers = filterOffers(
      offers,
      selectedOfferFilters,
      markets
    );
  ...
  filterOffers(type, value) {
    ...
    this.selectedOfferFilters = updateSelectedFilter(
      selectedOfferFilters,
      newFilter
    );
  ...
  onSortingChange(sort) {
    ...
    if (
      ...
      sort.target.attributes.length &&
      sort.target.attributes[0].value
    ) {
    ...
    if (searchParams && Object.entries(searchParams).length) {
      ...
      if (flexiblemonth) {
        ...
        if (
          ...
          urlStartDate.isSameOrAfter(getMoment(), "day")
        ) {
          filterStartDate = urlStartDate;
        }
        if (
          urlEndDate.isValid() &&
          urlEndDate.isSameOrAfter(urlStartDate, "day")
        ) {
