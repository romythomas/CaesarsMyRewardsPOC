...
const Auth = {
  ...
  login: () => requests.get("stubs/accountinfobyToken.json?callback=foo"),
...
const Profile = {
  ...
  getGuestProfile: (winnetid) =>
    requests.get(
      `stubs/getguestprofile.json?winnetid=${encode(winnetid)}&callback=foo`
    ),
  getFeeds: () => requests.get(`stubs/crwtiersprogress.json?callback=foo`),
};
